...
import {
  ArrowLeft, Edit2, Mail, Phone, MapPin, Calendar, CheckSquare, Square,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex-shrink-0">
        ...
        {/* Onboarding/Offboarding progress bar */}
                  ...
                  <div className="space-y-2">
                    {person.onboardingTasks.map((task) => (
                      ...
                      >
                        ...
                        <span className={clsx('text-sm flex-1', task.completed ? 'line-through text-gray-400' : 'text-gray-700')}>
                          {task.task}
                        </span>
                        <span className={clsx('text-xs px-2 py-0.5 rounded-full font-medium', taskCategoryColor[task.category])}>
                          {task.category}
                        </span>
                        {task.dueDate && (
                  ...
                  <div className="space-y-2">
                    {person.offboardingTasks.map((task) => (
                      <div
                        ...
                        className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:bg-orange-50 cursor-pointer transition-colors"
                      >
                        {task.completed
                          ? <CheckCircle size={16} className="text-green-500 flex-shrink-0" />
                          : <Square size={16} className="text-gray-300 flex-shrink-0" />
              ...
              <div className="card p-5">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-gray-900">{person.assetsAssigned}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-gray-900">{person.licensesAssigned}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <DollarSign size={14} className="text-green-500" />
                      <span className="text-sm text-gray-600">Annual IT Cost</span>
                    </div>
                    <span className="text-sm font-bold text-gray-900">${(person.totalItCost || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="divide-y divide-gray-50">
                {assignedAssets.map((asset) => (
                  <div
                    ...
                    className="flex items-center gap-4 py-3 hover:bg-gray-50 cursor-pointer rounded-lg px-2 -mx-2 transition-colors"
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">{asset.tag} · {asset.make} {asset.model}</p>
                    </div>
              ...
              {apps.slice(0, person.licensesAssigned).map((app) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500">{app.vendor} · {app.licenseType}</p>
                  </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-3 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-700">Hardware Assets</span>
                </div>
                <span className="text-sm font-semibold text-gray-900">
                  ${assignedAssets.reduce((s, a) => s + a.cost, 0).toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between py-3 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  <AppWindow size={15} className="text-purple-500" />
                  <span className="text-sm text-gray-700">Software Licenses (annual)</span>
                </div>
                <span className="text-sm font-semibold text-gray-900">
                  ${apps.slice(0, person.licensesAssigned).reduce((s, a) => s + a.costPerLicense * 12, 0).toFixed(0)}
                </span>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-semibold text-gray-900">Apply Onboarding Kit</h2>
            </div>
            <div className="p-6 space-y-3">
              {mockOnboardingKits.map((kit) => (
                <div
                  key={kit.id}
                  onClick={() => {
                    addToast({ type: 'success', message: `"${kit.name}" applied to ${person.name}` });
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {kit.assets.map((a, i) => (
                        <span key={i} className="text-xs bg-blue-100 text-blue-700 rounded px-1.5 py-0.5">{a.model}</span>
                      ))}
                      {kit.apps.map((a) => (
                        <span key={a.appId} className="text-xs bg-purple-100 text-purple-700 rounded px-1.5 py-0.5">{a.appName}</span>
                      ))}
                    </div>
                  </div>
                </div>
